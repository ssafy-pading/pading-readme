...
1. 화살표로 연결하기 (Prototype 모드 활용)
- 상단 메뉴에서 Prototype 탭으로 전환합니다.
- 연결할 UI 요소(예: 버튼)를 선택하고, 드래그하여 이동할 프레임으로 연결선을 만듭니다.
- 연결선의 끝에 나타나는 설정을 통해 다음을 설정합니다:
- Interaction: On Click (클릭 시 전환)
- Animation: Instant, Smart Animate, 또는 Dissolve 등 전환 효과 선택
2. 플로우 이름 설정
- 플로우의 시작점을 명확히 하기 위해 시작 화면 또는 첫 번째 프레임을 선택한 상태에서 상단의 Set as Starting Frame 버튼을 클릭합니다.
- 시작점을 기준으로 사용자 흐름이 자연스럽게 진행되도록 설계합니다.
3. 플로우 정리 및 주석 추가
- 플로우를 더 명확히 하기 위해 다음을 추가합니다:
- 텍스트 주석: 흐름이나 특정 요소의 설명을 추가하려면 텍스트 도구를 사용합니다.
- 화살표 및 선(Line Tool): 특정 동작 흐름을 시각적으로 연결합니다.
4. 플로우 테스트
- 우측 상단의 Play 버튼을 클릭하여 플로우를 테스트합니다.
- 화면이 정상적으로 전환되고 플로우가 매끄럽게 작동하는지 확인합니다.

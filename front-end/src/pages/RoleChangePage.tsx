...
const UserRoleChangePage: React.FC = () => {
  ...
  const handleRoleUpdate = async (userId: number) => {
    const newRole: string | undefined = selectedRoles[userId];
    if (!groupId || !newRole) {
  ...
  return (
    <div className={`transition-all duration-700 ${isProfileNavOpen ? "ml-64" : "ml-0"}`}>
      <div className="prelative pl-8 pr-12 pb-6 h-screen overflow-y-hidden max-h-screen transition-all duration-1000 ml-32 mt-20 z-0">
        <h1 className="text-[#4D4650] text-xl font-bold mb-3">멤버 설정</h1>
        <hr />
        <table className="w-full table-auto border-collapse border-spacing-0 text-sm mt-10">
...
const WrappedRoleChangePage: React.FC = () => {
  return (
    <NavigationProvider>
      <UserRoleChangePage />
    </NavigationProvider>
...
export default WrappedRoleChangePage;
